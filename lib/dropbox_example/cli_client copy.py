...
class DropboxTerm(cmd.Cmd):
    ...
    def __init__(self, app_key, app_secret, db_usr, db_pwd):
    ...
    def send_url(self, url):
        ...
        manager.add_password(None, url,  self.db_usr, self.db_pwd)
        ...
        req             = urllib2.Request(url, headers = {'Accept' : 'application/html'})

        result          = director.open(req)
        print result

        #print result.info() # result.info() will contain the HTTP headers
        #print result.read() # result.read() will contain the data
